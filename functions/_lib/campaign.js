export const DEFAULT_PARTNER_LINK =
  "https://2026-nexo-polic-y-fund.netlify.app/?partner_code=johanju";

export function normalizeCampaignInput(payload) {
  return {
    ...
    offerLink: normalizeText(payload?.offerLink) || DEFAULT_PARTNER_LINK,
    tone: normalizeText(payload?.tone) || "전문적이지만 친근한 톤",
    objective: normalizeText(payload?.objective) || "문의 전환",
    offerType: normalizeText(payload?.offerType) || "service",
    offerProvider: normalizeText(payload?.offerProvider),
    offerCategory: normalizeText(payload?.offerCategory),
    offerSummary: normalizeText(payload?.offerSummary),
    disclosureText: normalizeText(payload?.disclosureText),
    seedTags: Array.isArray(payload?.seedTags)
      ? payload.seedTags.map((tag) => normalizeText(tag)).filter(Boolean)
      : [],
  };
}
...
export function buildCampaignPrompt(input) {
  return [
    ...
    input.offerType ? `오퍼 타입: ${input.offerType}` : "",
    input.offerProvider ? `오퍼 공급처: ${input.offerProvider}` : "",
    input.offerCategory ? `오퍼 카테고리: ${input.offerCategory}` : "",
    input.offerSummary ? `오퍼 요약: ${input.offerSummary}` : "",
    input.disclosureText
      ? `경제적 이해관계 고지 문구: ${input.disclosureText}`
      : "",
    ...
    "4. threads.post는 짧고 강한 문제제기형 포스트로 작성한다.",
    ...
    input.disclosureText
      ? "11. 경제적 이해관계 고지는 숨기지 말고 자연스럽고 명확하게 반영한다."
      : "",
  ]
    .filter(Boolean)
    .join("\n");
...
export function normalizeCampaign(rawCampaign, input) {
  const baseTags = buildBaseTags(input);
  const blogTitle =
    normalizeText(rawCampaign?.blog?.title) || `${input.keyword} 실전 가이드`;
  ...
  const normalizedBlogBody = applyDisclosure(
    ensureLink(blogBody, input.offerLink),
    input.disclosureText,
  );
  const normalizedInstagramCaption = applyDisclosure(
    ensureLink(instagramCaption, input.offerLink),
    input.disclosureText,
  );
  const normalizedThreadsPost = applyDisclosure(
    ensureLink(threadsPost, input.offerLink),
    input.disclosureText,
  );
  const normalizedYoutubeDescription = applyDisclosure(
    ensureLink(youtubeDescription, input.offerLink),
    input.disclosureText,
  );
  const normalizedShortsCaption = applyDisclosure(
    ensureLink(normalizeText(rawCampaign?.shorts?.caption), input.offerLink),
    input.disclosureText,
  );
  const normalizedShortsScript = applyDisclosure(
  ...
  return {
    ...
    instagram: {
      caption: normalizedInstagramCaption,
    ...
    youtube: {
      ...
      description: normalizedYoutubeDescription,
    ...
    shorts: {
      ...
      caption: normalizedShortsCaption,
      script: normalizedShortsScript,
      ...
      fullText: buildShortsText(
        ...
        normalizeText(rawCampaign?.shorts?.hook) ||
          `${input.audience}라면 이 변화부터 보셔야 합니다.`,
        normalizedShortsCaption,
        normalizedShortsScript,
...
function buildBaseTags(input) {
  return [
    ...
    input.audience,
    input.offerCategory,
    input.offerProvider === "coupang" ? "쿠팡파트너스" : "",
    input.offerProvider === "tenping" ? "텐핑" : "",
    ...input.seedTags,
  ]
...
function applyDisclosure(text, disclosureText) {
  const normalized = normalizeText(text);
  const disclosure = normalizeText(disclosureText);
...
function normalizeReplyList(replies, input) {
  ...
  return [
    ...
    `상세 내용은 링크에서 확인해 보세요: ${input.offerLink}`,
  ];
}
...
function normalizeScenes(scenes, input) {
  ...
  return [
    ...
    },
    {
      ...
      narration: `${input.keyword}는 단순한 유행이 아니라 실제 변화를 만드는 포인트입니다.`,
      caption: `${input.keyword}가 필요한 이유`,
    },
    {
      time: "12-22초",
      visual: `${input.productName} 핵심 장점 3가지 텍스트`,
      narration: `${input.productName}를 활용하면 시간, 비용, 전달력이 함께 개선됩니다.`,
      caption: "핵심 장점 3가지",
    },
    {
      time: "22-35초",
      visual: "전후 비교 또는 사용 장면",
      narration: "실제 적용 장면만 봐도 왜 많은 사람들이 먼저 확인하는지 바로 이해할 수 있습니다.",
      caption: "적용 전후 비교",
    },
    {
      time: "35-45초",
